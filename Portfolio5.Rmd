...
```{r}
# Loading packages
pacman::p_load(metafor, tidyverse, pastecs, dplyr, ggplot2, readxl, lmerTest)
...
# Using lmer to to the meta analysis
#first lmer
as.factor(WM2$StudyID)
lm_1 <- lmerTest::lmer(yi ~ 0 + TYPE_OF_TASK + (1|StudyID), data = WM2)

#We get an error message when trying to fit a linear mixed model to the data. This is because we only have one observation (effect size, yi) per grouping variable (StudyID).
